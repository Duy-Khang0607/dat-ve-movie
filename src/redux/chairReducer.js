...
const reducer = (state = initialState, action) => {
  switch (action.type) {
    // Tick chọn ghế và bỏ chọn ghế
    case "CHANGE_CHAIR": {
      // sao chép lại 1 mảng obj
      const data = [...state.chairSelectedList];
      const foundChair = data.findIndex(
        (item) => item.soGhe === action.payload.soGhe
      );
      if (foundChair !== -1) data.splice(foundChair, 1);
      else {
